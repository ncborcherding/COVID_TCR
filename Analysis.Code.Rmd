...
ref8 <- load.reference.map("./annotations/ref_LCMV_Atlas_mouse_v1.rds")
ref4 <- load.reference.map("./annotations/ref_LCMV_CD4_mouse_release_v1.rds")
    
for (i in seq_along(file_list)){
    tmp <-  Read10X(paste0("./data/", file_list[i], "/filtered_feature_bc_matrix"))
...
data.inventory <- read.delim("data.inventory.txt")
meta <- list[[]]
...
saveRDS(list, file = "./processedData/Fullintegrated_seuratObjects.rds")
```
...
#Sampling to allow for equal coimparisons for density
sampled <- df %>%
  group_by(timepoint) %>%
  sample_n(25000)
...
saveRDS(tfh, "./processedData/Fullintegrated_seuratObjects_TFH_TFHonly.rds")
```

#Figure 2A
```{r}
tfh <- readRDS("./processedData/Fullintegrated_seuratObjects_TFH_TFHonly.rds")
...
library(patchwork)
plot <- NULL
features <- sort(c("BCL6", "CXCR5", "CD274", "ICOS", "CCR7","CD40LG","IL6R","CXCL13", "MKI67", "FOXP3", "IL10", "SELL"))
...
write.csv(markers, "tfh_onlymarkers.csv")
```


```{r}
markers <- read.csv("tfh_onlymarkers.csv")
...
ES <- readRDS("./ProcessedData/enrichmentResults.rds")
...
list <- readRDS("./processedData/Fullintegrated_seuratObjects_TFH.rds")
...
saveRDS(conga, "./processedData/conga_tfh_seuratObject.rds")
...
#Figure 3B
```{r}
conga <- readRDS("./processedData/conga_tfh_seuratObject.rds")
