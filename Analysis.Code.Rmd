...
Looking at the relative density of cells by time point. In order to ensure a unbiased comparison, we randomly downsample cells to compare equal number of cells. 
...
Identifying and visualizing gene set enrichments for Tfh cells/clusters.
...
Examining the changes in gene set enrichment as a function of cluster and time point
...
for(i in 3:44) { #min-max normalizes the gene sets
  summary[,i] <- normalize(summary[,i])
}
summary <- as.data.frame(summary)
...
Unique clonotype overlap as a function of cluster and time point.
```{r}
...
Relative density of cells by time point with the previously established spike-specific TCRs highlighted.
...
Co-embedding of clonotypes based on the PCA and TCR lament dimensions.
```{r}
...
Tfh markers across the clonotype manifold.
...
Overlay of spike-specific TCRs (TCRA and TCRB labels) on clonotype manifold.
...
TCR motifs in clusters with spike-specific clones.
...
Highlighting of clonotypes for validation. 
...
Examining spike-specific cells in the Tfh compartment.
...
sequence <- read.csv("./sequenceInfo/Sequences.csv")
CTaa <- as.data.frame(stringr::str_split(conga$CTaa, "_", simplify = TRUE))
TA <-which(CTaa$V1 %in% sequence$cdr3a)
TB <-which(CTaa$V2 %in% sequence$cdr3b)

conga$TCRB.S167.180<- NA
conga$TCRB.S167.180[TB] <- "Specific"

conga$TCRA.S167.180 <- NA
conga$TCRA.S167.180[TA] <- "Specific"
...
conga$TCRB.S815.830 <- NA
conga$TCRB.S815.830[TB] <- "Specific"

conga$TCRA.S815.830 <- NA
conga$TCRA.S815.830[TA] <- "Specific"
...
conga$TCRB.VDJ.db <- NA
conga$TCRA.VDJ.db <- NA
for (i in seq_along(epitope)) {
  sequences$Epitope[which(sequences$Epitope == names(epitope)[i])] <- epitope[i]
  TA <- sequences[sequences$Gene == "TRA" & sequences$Epitope == epitope[i], ]
  TB <- sequences[sequences$Gene == "TRB" & sequences$Epitope == epitope[i], ]
  TA.loc <-which(CTaa$V1 %in% TA$CDR3)
  TB.loc <-which(CTaa$V2 %in% TB$CDR3)
  conga$TCRB.VDJ.db[TB.loc] <- epitope[i]
  conga$TCRA.VDJ.db[TA.loc] <- epitope[i]
}

conga$TCRB.epitope  <- NA
conga$TCRB.epitope <- ifelse(!is.na(conga$TCRB.S167.180), "S167.180", NA)
conga$TCRB.epitope <- ifelse(!is.na(conga$TCRB.S815.830), "S815.830", conga$TCRB.epitope)
conga$TCRB.epitope <- ifelse(!is.na(conga$TCRB.VDJ.db) & is.na(conga$TCRB.epitope), conga$TCRB.VDJ.db, conga$TCRB.epitope)

conga$TCRA.epitope  <- NA
conga$TCRA.epitope <- ifelse(!is.na(conga$TCRA.S167.180), "S167.180", NA)
conga$TCRA.epitope <- ifelse(!is.na(conga$TCRA.S815.830), "S815.830", conga$TCRA.epitope)
conga$TCRA.epitope <- ifelse(!is.na(conga$TCRA.VDJ.db) & is.na(conga$TCRA.epitope), conga$TCRA.VDJ.db, conga$TCRA.epitope)

conga$TCRA.COVID <- "No"
conga$TCRA.COVID[grepl("Spike", conga$TRA_Epitope.target)] <- "Yes"
conga$TCRA.COVID[!is.na(conga$TCRA.epitope)] <- "Yes"
conga$TCRA.COVID[!is.na(conga$confirmation.neighbors.TCRA)] <- "Yes"
...
conga$TCRB.COVID[!is.na(conga$confirmation.neighbors.TCRB)] <- "Yes"
...
meta <- SeuratObj[[]]
meta <- meta[meta$CTaa %in% unique(c(TCRB.ref$CTaa, TCRA.ref$CTaa)),]
write.csv(meta, "./output/Figure4/Figure4_spike.clones_table.csv", row.names = FALSE)
...
Differential gene expression in spike-specific Tfh cells comparing early (d28) to late (d201).
...
```{r}
FvL.markers <- read.csv("./output/Figure4/Figure4A_Tfh.DEG.d28vsd201.csv")
colnames(FvL.markers)[1] <- "genes"

FvL.markers <- FvL.markers %>%
                mutate(Percent.Delta = abs(pct.1 - pct.2)*100)
...
FvL.plot <- ggplot(FvL.markers, aes(x = avg_log2FC, y = -log10(p_val))) + 
  ...
  theme_classic() + 
  scale_size(range = c(0.1,3), breaks = c(5,10,15,20,25,30)) + 
...
ggsave("./output/Figure4/Figure4A.pdf", height = 4.5, width = 5)
```

## Figure 4B

Examining gene set enrichment in spike-specific Tfh cells as a function of time.
...
pathways.intersect <- pathways.intersect[!grepl("REACTOME", pathways.intersect) ]
pathways.intersect <- pathways.intersect[-which(pathways.intersect %in% c( "BIOCARTA_FIBRINOLYSIS_PATHWAY", "BIOCARTA_AMI_PATHWAY", "BIOCARTA_EXTRINSIC_PATHWAY", "BIOCARTA_CREM_PATHWAY"))]
...
pheatmap::pheatmap(t(summary[,2:70]), scale = "row")
dev.off()
```

## Figure 4C 

Alluvial plots spike-specific clones in Tfh across time points. Y-axis is the relative proportion the clone represents as a function of the entire repertoire sequenced at the indicated time point.
...
Rarefication and extrapolation analysis for donors with blood and lymph node samples.
...
Venn diagram of clones in the blood and lymph nodes.
...
Scatter plot of clones in the blood and lymph nodes of donor 368-01a and 368-22 (donors with overlapping clonotypes).
...
Differential expression of spike-specific CD4+ T cells between lymph node and blood.
...
LNvB.markers <- read.csv("./output/Figure6/Figure6A_Ag.specific.LNvsBL.csv")
colnames(LNvB.markers)[1] <- "genes"
...
LNvBL.plot <- ggplot(LNvB.markers, aes(x = avg_log2FC, y = -log10(p_val_adj))) + 
  ...
  theme(axis.title = element_blank(), 
        axis.text = element_blank())
ggsave("./output/Figure6/Figure6A.pdf", height = 4.5, width = 5)
```

## Figure 6B

Examining differential enrichment in spike-specific CD4+ T cells between lymph node and blood.
...
UMAP embedding of spike-specific CD4+ T cells in COVID vaccination and infection.

```{r}
list <- readRDS("./data/processedData/PB_Fullintegrated_seuratObjects_Tcells.rds")
spike <- read.csv("./output/Figure7/Reference_spike.clones.csv")
...
Breakdown of UMAP/cluster of spike-specific CD4+ T cells in COVID vaccination and infection.
...
subset <- subset(subset.Obj, relative.time == "Late")
prop_test <- sc_utils(subset)

perm.results  <- permutation_test(
                  	prop_test, cluster_identity = "seurat_clusters",
                  	sample_1 = "s368", sample_2 = "s350",
                  	sample_identity = "study")
write.csv(perm.results@results$permutation, "./output/Figure7/cluster.permutation.results.csv")
```

## Figure 7D

Display of cluster markers.
...
Clusters of spike-specific clones as a function of vgene usage and edit distance. 
...
df <- df %>%
  ...
  mutate(n = n())
df$TRA_cluster[df$n >= 2] <- NA #removing any cluster that is 2 or less 
...
Breakdown of spike-specific CD4+ T cells in COVID vaccination and infection by time.
