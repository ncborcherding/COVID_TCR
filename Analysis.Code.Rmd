...
DimPlot(tfh, group.by = "cloneType") + 
  ...
  theme_void() + 
  theme(plot.title = element_blank())
ggsave("./output/Figure2/Figure2E.1_TFHonly.pdf", height = 3, width = 6)
...
write.csv(EvL.markers, "./output/Figure4/Figure4A_Tfh.DEG.EarlyvsLate.csv")
...
load("Trex.database.rda")
names(Trex.database) <- c("TRA", "TRB")

Trex.database[[1]] <- as.data.frame(Trex.database[[1]])
names(Trex.database[[1]]) <- c("CDR3", "V.GENE", "D.GENE", "J.GENE", "EPITOPE.SEQ", "EPITOPE.TARGET", "EPITOPE.SPECIES", "MHC.A", "MHC.B", "TISSUE", "CELLTYPE", "PMID", "DB")
Trex.database[[1]] <- unique(Trex.database[[1]])

Trex.database[[2]] <- as.data.frame(Trex.database[[2]])
names(Trex.database[[2]]) <- c("CDR3", "V.GENE", "D.GENE", "J.GENE", "EPITOPE.SEQ", "EPITOPE.TARGET", "EPITOPE.SPECIES", "MHC.A", "MHC.B", "TISSUE", "CELLTYPE", "PMID", "DB")      
Trex.database[[2]] <- unique(Trex.database[[2]])
...
annotateDB <- function(dat,
                       chains = "TRB") {
  relevent.db <- as.data.frame(Trex.database[[chains]])
  relevent.db <- relevent.db[,c("CDR3", "EPITOPE.TARGET", "EPITOPE.SEQ", "EPITOPE.SPECIES", "TISSUE", "CELLTYPE")]
  
  colnames(relevent.db)[2:6] <- paste0(chains, "_", colnames(relevent.db)[2:6])
  dat <- merge(dat, relevent.db, by.x =chains, by.y = "CDR3", all.x = TRUE)
}

meta <- annotateDB(meta, chains = "TRA")
meta <- annotateDB(meta, chains = "TRB")

meta$CMV.reactive <- "Neg"
meta$CMV.reactive[grepl("CMV",meta$TRA_EPITOPE.SPECIES)] <- "Pos"
meta$CMV.reactive[grepl("CMV",meta$TRB_EPITOPE.SPECIES)] <- "Pos"

meta$EBV.reactive <- "Neg"
meta$EBV.reactive[grepl("EBV",meta$TRA_EPITOPE.SPECIES)] <- "Pos"
meta$EBV.reactive[grepl("EBV",meta$TRB_EPITOPE.SPECIES)] <- "Pos"
...
colnames(dat.output) <- c("subjects", "TRA.intersect", "TRA.union", "TRA.blood", "TRA.LN", 
                          "TRB.intersect", "TRB.union", "TRB.blood", "TRB.LN",
                          "combined.intersect", "combined.union", "combined.blood", "combined.LN")
write.csv(dat.output, "TCRsummary.csv")
...
for (i in seq_along(subjects)) {
  ...
  for(j in seq_along(chain.combo)) {
    ...
    venn.plot <- plot(fit, 
         quantities = TRUE,
         labels = list(font = 4))
    pdf(paste0("./output/Figure4/Figure5B_", subjects[i], ".", chain.combo[j], "_allTCRs.pdf"), height = 3, width = 3.5)
      print(venn.plot)
    dev.off()
  }
}
...
```{r}
list <- readRDS("./processedData/PB_Fullintegrated_seuratObjects_Tcells.rds")
