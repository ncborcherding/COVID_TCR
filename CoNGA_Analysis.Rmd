...
subset.conga <- subset(subset.conga, CTaa %in% conga.nn.idx$CTaa)
...
for (i in seq_along(clones)) {
  ...
  denominator <- length(unique(conga.nn, Trex.nn))
  if (numerator > 0) {
...
df1.1 <- data.frame(sum = rowSums(table(tcrdist$KF_snn_res.0.3, tcrdist$TCRA.epitope, useNA = "ifany")))
df1.1$Var1 <- rownames(df1.1)

trex.break <- ggplot(df1, aes(x= Var1, y = Freq)) + 
  geom_bar(stat = "identity", position = "fill", aes(fill = Var2)) + 
  geom_text(df1.1, mapping = aes(label = sum, x = Var1, y = 1.1),  size = 2) + 
...
conga.break <- ggplot(df2, aes(x= Var1, y = Freq)) + 
                geom_bar(stat = "identity", position = "fill", aes(fill = Var2)) + 
                geom_text(df2.1, mapping = aes(label = sum, x = Var1, y = 1.1), size = 2) + 
                scale_fill_manual(values = ep.colors, na.value = "grey") + 
                theme_classic() + 
                scale_x_discrete(limits = rev) + 
