...
TRA <- data.frame(CT = stringr::str_split(conga$CTaa, "_", simplify = TRUE)[,1], 
...
TRB <- data.frame(CT = stringr::str_split(conga$CTaa, "_", simplify = TRUE)[,2], 
...
ADT <- merge.sparse(TRB.adj.matrix)
x <- as(TRB.clust.list[[i]]$adj_mtx, "dMatrix")
```
