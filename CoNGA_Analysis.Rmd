...
conga <- FindNeighbors(conga, 
                      reduction = "KF.only", 
                       k.param = 11, 
                       return.neighbor = TRUE)
Trex.nn.idx <- as.data.frame(conga@neighbors$RNA.nn@nn.idx[,2:11])

#Trex.nn.idx$CTaa <- subset.conga$CTaa
Trex.nn.idx$CTaa <- conga$CTaa
...
for (i in seq_along(clones)) {
  ...
  cg.tmp <- as.numeric(unique(unlist(conga.nn.idx[conga.nn.idx$CTaa == clones[i],1:10])))
  conga.nn <- conga.nn.idx$CTaa[cg.tmp]
  numerator <- length(intersect(Trex.nn, conga.nn))
  denominator <- length(unique(c(conga.nn, Trex.nn)))
  #Filtering leads to a reduction in conga, that needs to be calculated
  if (numerator > 0) {
...
ggplot(tcrdist, aes(x= X1, y = X2)) + 
                    stat_density_2d(
                      geom = "polygon", 
                      bins = 25,
                      aes(fill = stat(level))) +
                    facet_wrap(~clusters_tcr) +
                    xlab("gexUMAP_1") + 
                    ylab("gexUMAP_2") + 
...
x <- igraph::graph_from_adjacency_matrix(TRA.sparse, mode = "undirected")
x <- igraph::simplify(x)

V(x)
plot.layout <- layout_with_mds(x)
plot(x,
     vertex.label = NA, 
     layout = plot.layout,
     size = 8, 
...
y <- cluster_fast_greedy(x)

plot(y,x, vertex.label = NA,)

#Handle the duplicates sequences across several
#Need to figure out neighborhood ID
