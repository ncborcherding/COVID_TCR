...
summary <- summary[,-c(which(colSums(summary[2:25]) == 0)+1)]
...
pheatmap::pheatmap(t(summary[,2:23]))
dev.off()
```
...
for(i in seq_along(duplicated)) {
  ...
  return.cc$Clonotypes <- as.vector(return.cc$Clonotypes)
  ...
  ggplot(return.cc, aes(x = Sample, 
                ...
                geom_stratum() +
                geom_flow(stat = "alluvium", aes.bind = TRUE) + 
                guides(fill = "none")  + 
